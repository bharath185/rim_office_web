...
@Component({
  ...
  templateUrl: './teamsleave-hr.component.html',
  styleUrl: './teamsleave-hr.component.scss'
})
export class TeamsleaveHrComponent implements OnInit {
  ...
  toDate: string = '';
  status: string = '';
  combinedLeaveDates = new Set<string>();
  selectedDate: string = '';
  ...
  loadAllHRLeaveData() {
    ...
    this.leaveSerive.GetAllApplyHRLeave(reqBody).pipe(
      switchMap(applied =>
        of(null).pipe(
          delay(1000),
          switchMap(() =>
            this.leaveSerive.GetAllHRLeave(reqBody).pipe(
  ...
  approveSelectedByHR() {
    ...
    console.log(reqBody);
    this.isSpinner = true;
    this.leaveSerive.ApproveLeaveByHR(reqBody).subscribe({
      next: (res: any) => {
        if (res['msg']) {
  ...
  rejectSelectedByHR() {
    ...
    console.log(reqBody)
    this.isSpinner = true;
    this.leaveSerive.RejectLeaveByHR(reqBody).subscribe({
      next: (res: any) => {
        if (res['msg']) {
  ...
  filterAllListByStatus(status: string) {
    if (!status) {
      this.rowsAllListHR = [...this.originalRowsAllListHR];
      console.log(this.rowsAllListHR)
    } else {
      this.rowsAllListHR = this.originalRowsAllListHR.filter((row: any) =>
